...
class MarioCharacter:
    ...
    def draw(self, screen, camera_x):
        """Draw Mario with authentic Game Boy graphics"""
        screen_x = self.x - camera_x
        if screen_x + self.width < 0 or screen_x > DISPLAY_WIDTH:
            return
            
        # Draw Mario using authentic Game Boy 4-color palette
        # Body (dark gray)
        ...
        pygame.draw.rect(screen, GB_COLORS['lightest'],
                        (screen_x + 4, self.y + 8, self.width - 8, self.height - 8))
        ...
        # Mustache (darkest)
        pygame.draw.rect(screen, GB_COLORS['darkest'],
                        (screen_x + 4, self.y + 8, self.width - 8, 2))
...
class GameBoySimulatorGUI:
    ...
    def create_virtualbox_ui(self):
        ...
        view_menu.add_command(label="Authentic Mode", command=self.toggle_authentic)
    ...
    def toggle_authentic(self):
        """Toggle authentic mode"""
        messagebox.showinfo("Authentic Mode", "Running in authentic Game Boy mode with 4-shade grayscale")
